...
The chart will automatically create a self-signed cert-manager `Issuer` and a `Certificate` for the REST API. The resulting TLS `Secret` is mounted into the `api-rest` pod and REST clients.
...
To override the generated Secret name:

```console
$ helm install mayastor . -n <mayastor-namespace> \
  --set tls.mode=certManager \
  --set tls.certManager.secretName=<rest-tls-secret-name>
```
