...
| io_engine.&ZeroWidthSpace;cpuManagerAware | Derive the reactor cores from the cgroup cpuset assigned by the kubelet CPU manager instead of a statically configured core list. The -l flag is then omitted from the rendered args and computed at container start, which keeps SPDK reactors aligned with the cores Kubernetes actually granted. Requires kubelet cpuManagerPolicy=static and a QoS class Guaranteed pod with an integer CPU count, otherwise the container refuses to start. | `false` |
...
| io_engine.&ZeroWidthSpace;maxReactors | Upper bound on the number of reactors accepted in cpuManagerAware mode. Guards against a misconfigured pod inheriting the whole shared CPU pool. | `8` |
