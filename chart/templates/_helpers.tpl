...
Both selfSigned and certManager modes expose the HTTPS endpoint on port 8080.
*/}}
...
For certManager mode an optional custom name can be set via tls.certManager.secretName.
For selfSigned mode the name is always derived from the release name.
*/}}
...
{{- else -}}
{{- printf "%s-api-rest-tls" .Release.Name -}}
{{- end -}}
{{- end -}}

{{/*
Returns the in-container CA certificate file path for api-rest TLS clients.
The Secret (whether generated by Helm or cert-manager) exposes the CA under the ca.crt key.
*/}}
