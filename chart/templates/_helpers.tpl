...
{{/*
Returns the "--grpc-auto-tls" argument when gRPC TLS is enabled.
This will eventually be used only when using the transient pod-engine certificate (autoGenerated engine=pod).
Emits nothing otherwise.
*/}}
{{- define "grpc_auto_tls_arg" -}}
...
{{/*
Returns the URL scheme for in-cluster gRPC endpoints.
TLS enabled: https. TLS disabled: http.
*/}}
{{- define "grpc_scheme" -}}
{{- if .Values.security.tls.enabled -}}https{{- else -}}http{{- end -}}
{{- end -}}
