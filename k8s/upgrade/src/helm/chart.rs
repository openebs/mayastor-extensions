...
use k8s_openapi::api::core::v1::{Container, EnvVar, Probe, Volume, VolumeMount};
use semver::Version;
...
impl CoreValues {
    ...
    /// its '--storage.path' argument. The loki.write component's write-ahead log is written
    /// to a sub-directory of this path.
    ...
    /// This is a getter for the environment variables which are added to the alloy
    /// container, over and above the ones which the alloy helm chart adds by itself.
    pub(crate) fn alloy_extra_env(&self) -> &[EnvVar] {
        self.alloy.extra_env()
    }
    ...
    /// This returns the image object from the .initContainers yaml key.
    pub(crate) fn base_init_containers_image(&self) -> Option<&InitContainersImage> {
        self.base.init_containers_image()
    }
...
impl Base {
    ...
    /// This returns the image object from the .initContainers yaml key.
    fn init_containers_image(&self) -> Option<&InitContainersImage> {
        self.init_containers.image()
    }
...
struct InitContainers {
    image: Option<InitContainersImage>,
    containers: Vec<Container>,
}

impl InitContainers {
    fn image(&self) -> Option<&InitContainersImage> {
        self.image.as_ref()
    }
...
#[derive(Default, Deserialize)]
pub(crate) struct InitContainersImage {
    name: String,
    tag: String,
}

impl InitContainersImage {
...
impl Alloy {
    ...
    /// This is a getter for the alloy container's extra environment variables.
    fn extra_env(&self) -> &[EnvVar] {
        self.alloy.extra_env()
    }
...
struct AlloyAgent {
    ...
    storage_path: String,
    /// These are the environment variables which are added to the alloy container, over and
    /// above the ones which the alloy helm chart adds by itself.
    #[serde(rename(deserialize = "extraEnv"))]
