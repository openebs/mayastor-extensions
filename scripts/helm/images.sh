...
upgrade_job_image() {
  local images="${1:-}"
  local callhome_image

  # ie docker.io/openebs/mayastor-obs-callhome:v2.13.0
  callhome_image="$(grep -m1 "/mayastor-obs-callhome:" "$images")"
  [ -z "$callhome_image" ] && log_fatal "Can't find the callhome image in $images"

  # ie docker.io/openebs/mayastor-upgrade-job:v2.13.0
  echo "${callhome_image/mayastor-obs-callhome:/mayastor-upgrade-job:}"
}
...
helm_on_demand_images() {
  ...
  upgrade_job_image "$images" >> "$images"
}
