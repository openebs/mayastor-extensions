...
helm_dep_value() {
  ...
  value="$($HELM show values "$CHART_DIR" 2>/dev/null | yq -r ".$dep_name.$key")"
  ...
  value="$($HELM show values "$dep_chart" 2>/dev/null | yq -r ".$key")"
  if [ $? -ne 0 ]; then
    log_fatal "Can't the helm dependency $dep_name: $dep_chart"
  fi
  if [ -n "$value" ] && [ "$value" != "null" ]; then
    echo "$value"
    return 0
...
helm_dep_version() {
  $HELM show chart "$CHART_DIR" 2>/dev/null | dep="${1:-}" yq '.dependencies[]|select(.name == strenv(dep)).version'
}
...
helm_localpv_prov_helper_image() {
  ...
  if ! [ -f "$chart" ]; then
    log_fatal "Can't the helm dependency: $name"
  fi
...
case "$COMMAND" in
  generate)
    ...
    $HELM template "$CHART_DIR" --set "$ENABLE_ANALYTICS" 2>/dev/null | grep -Po "^[ \t]*image: \K(.*:.*)$" | tr -d \" | sort | uniq > "$TEMPLATE_IMAGES"
    ...
    cat "$TEMPLATE_IMAGES" | sort | uniq > "$IMAGES"
  ...
  verify)
    ...
    kubectl -n "$NAMESPACE" get pods -o json | jq -r '.items[].spec.containers[]?.image, .items[].spec.initContainers[]?.image' | sort | uniq > "$LIVE_IMAGES"
    #echo "bob" >> "$LIVE_IMAGES"
    #echo "kakaka" >> "$LIVE_IMAGES"
    while IFS= read -r image; do
